...
module Notifications
  ...
  # 読まない（docs/spec.md 11.）。「全員が対応済み」は定義できず、0冊かどうかで
  # 測れるのは「まったく手を付けていない」だけになる。
  #
  # 本文に載せるのは交換会名・締切・リンクだけにする。チャンネルへの投稿は誰が読むかを
  # 選べないので、ギフトコードと個人の希望リストは載せられない（docs/spec.md 8.）
  class DeadlineReminder
    ...
    # 締切を持つフェーズ。準備中が待っているのは締切ではなく登録期間の開始で、
    # まだ促す行動が無い。マッチング実行待ちが待っているのは主催者の操作、
    # 結果公開はもう終わっている（docs/spec.md 4.）。
    ...
    # 走査の途中で読み直すと、窓の縁にいる交換会を前半と後半で違う扱いにする
