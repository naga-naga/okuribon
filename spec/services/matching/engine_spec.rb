...
RSpec.describe Matching::Engine do
  ...
  shared_examples "マッチングの不変条件を満たす" do
    ...
    it "各参加者の取得数が取得枠と一致する" do
      ...
      participants.each do |participant|
        expect(taken.fetch(participant, 0)).to eq(result.quotas[participant]),
          "#{participant}: 取得 #{taken.fetch(participant, 0)}冊 / 枠 #{result.quotas[participant]}冊"
      end
    end
  ...
  context "標準的な構成（4人・各2冊・希望はばらける）" do
    let(:participants) { %w[A B C D] }
    let(:books) { build_books("A" => 2, "B" => 2, "C" => 2, "D" => 2) }
    let(:wishes) do
      {
        "A" => %w[B1 C1 D1 B2 C2 D2],
        "B" => %w[C1 D1 A1 C2 D2 A2],
        "C" => %w[D1 A1 B1 D2 A2 B2],
        "D" => %w[A1 B1 C1 A2 B2 C2]
      }
    end
    let(:result) { run_matching(participants: participants, books: books, wishes: wishes, seed: 12_345) }

    include_examples "マッチングの不変条件を満たす"
    ...
    it "全員が上位の希望から順に確保している" do
      expect(result.assignments.select(&:from_draft?).size).to eq(8)
    end
  end

  # ------------------------------------------------------------------
  context "冊数が偏り、受け取り手のない本が出る（A=5冊・B=1冊）" do
    let(:participants) { %w[A B] }
    let(:books) { build_books("A" => 5, "B" => 1) }
    let(:wishes) { { "A" => %w[B1], "B" => %w[A1 A2 A3 A4 A5] } }
  ...
  context "全員の希望が同じ本に集中し、すぐ枯渇する" do
    ...
    let(:wishes) { { "A" => %w[B1], "B" => %w[A1], "C" => %w[A1], "D" => %w[A1] } }
  ...
  context "希望を一切出さなかった参加者がいる" do
    let(:participants) { %w[A B C] }
    let(:books) { build_books("A" => 2, "B" => 2, "C" => 2) }
    let(:wishes) { { "A" => %w[B1 C1 B2 C2], "B" => %w[A1 C1 A2 C2] } }
  ...
  context "参加者が1人しかいない" do
    let(:participants) { %w[A] }
    let(:books) { build_books("A" => 3) }
    let(:wishes) { { "A" => %w[A1 A2 A3] } }
  ...
  describe "再現性" do
    ...
    it "同じ seed なら結果が完全に一致する" do
      expect(assignments_for(2024)).to eq(assignments_for(2024))
    end
  ...
  describe "ランダム入力による総当たり" do
    it "1000件すべてで不変条件を満たす" do
      ...
      1000.times do
        ...
        books = build_books(participants.to_h { |p| [ p, rand(0..4) ] })
        next if books.empty?

        book_ids = books.map(&:id)
        wishes = participants.to_h { |p| [ p, book_ids.select { rand < 0.6 }.shuffle ] }
        ...
        violations << "自分の本を受け取っている" unless result.assignments.all? { |a|
